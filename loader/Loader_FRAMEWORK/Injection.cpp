...
VOID CInjection::doInject()
{
	HRESULT hr;
	LPCTSTR Url = _T("Path to your dll host"), File = _T("C:\\ProgramData\\test\\test.dll"); //change
	hr = URLDownloadToFile(0, Url, File, 0, 0);

	proccessname = "csgo.exe";

	dllname = "C:\\ProgramData\\test\\test.dll"; //change

	manual_map->manualmapmain(proccessname.c_str(), dllname.c_str());
	
	Sleep(10);

	const int result = remove("C:\\ProgramData\\test\\test.dll"); //change
